...
static std::vector<Instruction*> getSuccsAndCalls(Module* M, Instruction *i) {
  std::vector<Instruction*> res = getSuccs(i);
  if (isa<CallInst>(i) || isa<InvokeInst>(i)) {
    CallSite cs(i);
    Function *f = getDirectCallTarget(cs);

    if (f) {
      if (!f->isIntrinsic()) {
        res.push_back(f->begin()->begin());
      }
    } else {
      /* indirect function calls unsupported */
    }
  }
  return res;
}
...
uint64_t klee::computeMinDistToUncovered(ExecutionState *es,
                                         const KInstruction *pki /* = NULL */)
{
  ...
  for (ExecutionState::stack_ty::iterator sfIt = es->stack.begin(),
    ...
    currentFrameMinDist = computeMinDistToUncovered((next == es->stack.end() && pki ? pki : kii), currentFrameMinDist);
  }
  return currentFrameMinDist;
}
...
bool StatsTracker::isCovered(const Instruction* i)
{
  unsigned id = executor.kmodule->infos->getInfo(i).id;
  return !theStatisticManager->getIndexedValue(stats::uncoveredInstructions, id);
}

void StatsTracker::getReturnInstructions(Value* v, std::vector<const ReturnInst*>& rets)
{
  CallSite cs(v);
  Function *f = getDirectCallTarget(cs);
  ...
  if (f && !f->isIntrinsic()) {
    for (Function::iterator fi = f->begin(), fe = f->end(); fi != fe; ++fi) {
      ...
      if (isa<ReturnInst>(ti))
        rets.push_back(dyn_cast<ReturnInst>(ti));
    }
  }
}
