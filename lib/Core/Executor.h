...
private:
  struct RegenInfo {
    RegenInfo(ref<Expr> c, const InstructionInfo* i,
              uint64_t inst, llvm::Value* llvmC,
              const std::vector<const MemoryObject*> deps,
              PTreeNode* p,
              int64_t eqL = 0)
      : cond(c)
      , ii(i)
      , instsOnPath(inst)
      , llvmCond(llvmC)
      , condDependencies(deps)
      , parent(p)
      , eqLiteral(eqL) { if (p) p->addRef(); }
    ref<Expr> cond;
    const InstructionInfo* ii;
    uint64_t instsOnPath;
    llvm::Value* llvmCond;
    std::vector<const MemoryObject*> condDependencies;
    PTreeNode* parent;
    int64_t eqLiteral;
  };

  struct InfeasibleLocation {
    InfeasibleLocation(unsigned int d, unsigned int i) :
      distance(d),
      iteration(i) { }
    unsigned int distance;
    unsigned int iteration;
    bool operator<(const InfeasibleLocation& rhs) const {
      return distance < rhs.distance || (distance == rhs.distance && iteration > rhs.iteration);
    }
  };
  ...
  typedef std::multimap<InfeasibleLocation, RegenInfo> InfeasibleStateMap;
  std::vector<ExecutionState*> stateChain;
  std::map<ExecutionState*, std::vector<SeedInfo> > shadowSeedMap;
  InfeasibleStateMap infeasibleStates;
  ...
  /// Used to maintain seeds across executions. Only used for short options
  std::map<std::string, std::vector<unsigned char> > persistentSeeds;
  ...
  /// Extra arguments added to satisfy getopt_long constraints
  std::vector<std::string> extraArgs;
  bool restartProgram;
  bool optionArgExists;
  ...
  void handleExternal(const std::string& name,
                      ExecutionState &state,
                      KInstruction *target,
                      std::vector< ref<Expr> > &arguments);
  ...
  void outputCoverageInfo(ExecutionState* state,
...
public:
  ...
  virtual void setHaltExecutionCP(bool value) {
    haltExecutionCP = value;
  }
  ...
  void adjustSeeds(ExecutionState &state, std::vector<SeedInfo>& seeds);
  void mergeSeeds(std::vector<SeedInfo> seeds, const std::vector<SeedInfo>& extraSeeds);
  void addInfeasibleInstruction(ExecutionState& state,
                                ref<Expr> cond,
                                PTreeNode* stateNode,
  ...
  bool compatibleForIPR(ExecutionState* target, ref<Expr> condition);
  ExecutionState* findIPRTarget(PTreeNode* node, ref<Expr> infeasibleCondition);
  void doInformedPathRegeneration(ExecutionState* es, ref<Expr> cond, const std::vector<SeedInfo>& currentSeeds);
  void doGreedy(ExecutionState* es, const std::vector<SeedInfo>& currentSeeds);

  bool findUncoveredDefinition(llvm::Value* cond, int64_t targetDef);
  void findDefinitions(llvm::Value* v, const llvm::BasicBlock* bb,
                       std::vector<const llvm::BasicBlock*>& bbDefs, bool& getopt,
                       llvm::CallInst*& getoptCall,
                       int64_t preferredValue = 0, DefType defType = DEF_EQ);

  void changeTarget(int assemblyLine = -1);
  bool argExists(const std::string& arg);
  std::string getOption(int64_t, llvm::GlobalVariable*, int& hasInitializer);
  ref<Expr> handleStrcmp(ExecutionState& state, ref<Expr> cond, unsigned strcmpIndex);

  void persistSeeds(ExecutionState& state);
