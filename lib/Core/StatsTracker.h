...
namespace klee {
  ...
  public:
    ...
    bool isCovered(const llvm::Instruction* i);
    void getReturnInstructions(llvm::Value* v, std::vector<const llvm::ReturnInst*>& rets);
  };

  uint64_t computeMinDistToUncovered(const KInstruction *ki,
                                     uint64_t minDistAtRA);
  uint64_t computeMinDistToUncovered(ExecutionState *state,
