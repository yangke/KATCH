...
#define addDNR(name, handler) { name, \
  ...
  add("klee_external", handleExternal, true),
...
void SpecialFunctionHandler::handleExternal(ExecutionState &state,
                                            KInstruction *target,
                                            std::vector<ref<Expr> > &arguments) {
  assert(arguments.size() > 1 && "invalid number of arguments to klee_external");
  std::string fn = readStringAtAddress(state, arguments[0]);
  std::vector<ref<Expr> > fnargs(arguments.begin() + 1, arguments.end());
  executor.handleExternal(fn, state, target, fnargs);
}
