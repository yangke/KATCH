...
cl::opt<double>
MaxCPTime("max-cp-time",
        cl::desc("Stop execution of the concrete path after the specified number of seconds (0=off)"),
        cl::init(0));
...
class CPHaltTimer : public Executor::Timer {
  Executor *executor;

public:
  CPHaltTimer(Executor *_executor) : executor(_executor) {}
  ~CPHaltTimer() {}

  void run() {
    std::cerr << "KLEE: CP HaltTimer invoked\n";
    executor->setHaltExecutionCP(true);
  }
};
...
void Executor::initTimers() {
  ...
  // In a swdef step, keep the original halt timer but reset the cp halt timer
  if (timers.size()) {
    timers.pop_back();
  } else {
    if (MaxTime) {
      ...
      if (!MaxCPTime)
        MaxCPTime = 0.9*MaxTime;
    }
  }
  if (MaxCPTime) {
    addTimer(new CPHaltTimer(this), MaxCPTime);
  }
}
