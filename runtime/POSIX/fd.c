...
static exe_disk_file_t *__get_sym_file_internal(const char *pathname, int flags, bool symbolic);
...
static exe_disk_file_t *__get_sym_file_internal(const char *pathname, int flags, bool symbolic) {
  ...
  if (symbolic) {
    ...
    bool symbolic_file = false;
    if (klee_is_symbolic(pathname[0])) {
      fprintf(stderr, "[KATCH] Opening symbolic file %s\n", pathname);
      symbolic_file = true;
    }
    if (symbolic_file) {
      ...
      for (i=0; i<__exe_fs.n_sym_files; ++i) {
        if (__exe_fs.sym_files[i].path && 0 == strcmp(__exe_fs.sym_files[i].path, pathname)) {
      ...
      fprintf(stderr, "[KATCH] Opening concrete file %s (symbolic = %d)\n", pathname, symbolic);
    }
  }
...
int __fd_open(const char *pathname, int flags, mode_t mode) {
  ...
  bool sandbox = false;
  /* XXX Hack to follow paths which require folders */
  if (flags & O_DIRECTORY) {
    struct stat64 buf;
#if __WORDSIZE == 64 
    int r = syscall(__NR_stat, pathname, &buf); 
#else 
    int r = syscall(__NR_stat64, pathname, &buf); 
#endif 
  ...
  df = __get_sym_file_internal(pathname, flags, (flags & O_ACCMODE) == O_RDONLY && !sandbox); 
  if (df) {    
...
int __fd_stat(const char *path, struct stat64 *buf) {  
...

  {
    ...
    if (r == -1)
      ...
      klee_make_symbolic(buf, sizeof(*buf), "stat");
      klee_assume(buf->st_size <= file_size);
    }
    return r;
  }
}
...
int __fd_lstat(const char *path, struct stat64 *buf) {
  ...
  {    
#if __WORDSIZE == 64
    int r = syscall(__NR_lstat, path, buf);
#else
    int r = syscall(__NR_lstat64, path, buf);
#endif
    if (r == -1)
      errno = klee_get_errno();
    else {
...
int __fd_fstat(int fd, struct stat64 *buf) {
  ...
  if (!f->dfile) {
    ...
    if (r == -1)
      ...
      off_t original_file_size, file_size = original_file_size = buf->st_size;
      klee_make_symbolic(&file_size, sizeof(file_size), "stat-st-size");
      klee_assume(file_size <= original_file_size);
...
int __fd_getdents(unsigned int fd, struct dirent64 *dirp, unsigned int count) {
  ...
  if (f->dfile) {
    ...
    memset(dirp, 0, count);
    //int s = syscall(__NR_lseek, f->fd, os_pos, SEEK_SET);
    //assert(s != (off64_t) -1);
    res = syscall(__NR_getdents64, f->fd, dirp, count);
    if (res == -1) {
      errno = klee_get_errno();
    } else {
      //f->off = syscall(__NR_lseek, f->fd, 0L, SEEK_CUR);
      int remaining = res;
      struct dirent64* dp = dirp;
      while (remaining) {
        size_t filesize = strlen(dp->d_name);
        if (filesize > 0) {
          char* p = (char*)malloc(filesize);
          memcpy(p, dp->d_name, filesize);
          klee_make_symbolic(p, filesize, "direntry");
          memcpy(dp->d_name, p, filesize);
          free(p);
        }
        remaining -= dp->d_reclen;
...
int inflate(z_streamp strm, int flush) {
  __concretize_buffer((char*)strm->next_in, strm->avail_in);
  Bytef* next_out = strm->next_out;
  uLong avail_out = strm->avail_out;

  Bytef* tmp = (Bytef*)malloc(avail_out);
  strm->next_out = tmp;
  int x = klee_external("inflate", strm, flush);
  if (x > 0) {
    if (avail_out != strm->avail_out) {
      klee_make_symbolic(tmp, avail_out, "zlib");
      memcpy(next_out, tmp, avail_out);
    }
  }
