...
static void __create_new_dfile(exe_disk_file_t *dfile, unsigned size, 
  ...
  if (content) {
    memcpy(dfile->contents, content, size);
  }
  fprintf(stderr, "[KATCH] file content is now symbolic\n");
  klee_make_symbolic(dfile->contents, dfile->size, name);
  if (content) {
    unsigned i;
    for (i = 0; i < dfile->size; ++i) {
      klee_prefer_cex(dfile->contents, dfile->contents[i] == content[i]);
    }
  }
...
void klee_init_fds(unsigned n_files /* unused */, unsigned file_length, 
  ...
  n_files = KATCH_SYM_FILES;
  __exe_fs.n_sym_files = n_files;
  __exe_fs.sym_files = malloc(sizeof(*__exe_fs.sym_files) * n_files);
  memset(__exe_fs.sym_files, 0, sizeof(*__exe_fs.sym_files) * n_files);
  
  /* setting symbolic stdin */
  if (file_length) {
    __exe_fs.sym_stdin = malloc(sizeof(*__exe_fs.sym_stdin));
    __create_new_dfile(__exe_fs.sym_stdin, file_length, "stdin", NULL, &s);
  ...
  if (sym_stdout_flag) {
    ...
    __create_new_dfile(__exe_fs.sym_stdout, 1024, "stdout", NULL, &s);
...
exe_disk_file_t* klee_init_cp_file(const char* path, int flags) {
  ...
  for (k=0; k < __exe_fs.n_sym_files; k++) {
    if (!__exe_fs.sym_files[k].stat) {
      __create_new_dfile(&__exe_fs.sym_files[k], fsize, path, buf, &def);

      __exe_fs.sym_files[k].path = (char*)malloc(strlen(path)+1);
      strcpy(__exe_fs.sym_files[k].path, path);
      free(buf);
      return &__exe_fs.sym_files[k];
    }
  }
  return NULL;
}
